...
exports.explore = async (req, res) => {
  try {
    ...
    data = await db.postview.findById({
      _id: "613b26eae8548fc8339af207"
    })
    shortVideosRaw = data.shortVideos
    allPostsRaw = data.allPosts
    userData = await db.user.findById(req.userData.userId).select('following')
    if (!data) {
      res.status(404).json({
        error: " No data to show.!"
      ...
      if (!shortVideosRaw) {
        console.log("Shortvideos not available")
      }
      if (!allPostsRaw) {
        console.log("AllPosts not available")
      }
      shortVideos = shortVideosRaw.reverse()
      allPosts = allPostsRaw.reverse()
      ...
      statusContent = await db.status.currentStatus(req.userData.userId)
      statusToshow = statusContent
      if (!statusToshow) {
        console.log("statusToShow not found.!")
      }
      ...
      if (dataType == "previous" && allPosts.includes(postId)) {
        ...
        usersResponse = await db.user.userArrayDetails(usersToShow)
        postResponse = await db.post.arrayPostDetail(allPosts.slice(postIndex + 1, postIndex + 21))
        shortVideoIndex = Math.round(postIndex / 20 * 10)
        shortVideosResponse = await db.post.arrayPostDetail(shortVideos.slice(shortVideoIndex + 1, shortVideoIndex + 11))

        // creating response
        responseArray.push(...postResponse.slice(0, 5))
        userBlock = {
          postContentType: "profileReference",
          title: 'People You may know',
          data: usersResponse
        ...
        responseArray.push(userBlock)
        responseArray.push(...postResponse.slice(5, 10))
        statusBlock = {
          postContentType: "statusReference",
          title: "Status",
          data: statusToshow
        }
        if (statusBlock.data.length > 0) {
          responseArray.push(statusBlock)
        }
        responseArray.push(...postResponse.slice(10, 15))
        shortVideoBlock = {
          postContentType: "shortVideoReference",
          title: "Short Videos",
          data: shortVideosResponse
        }
        responseArray.push(shortVideoBlock)
        responseArray.push(...postResponse.slice(15, 20))
        ...
        usersResponse = await db.user.userArrayDetails(usersToShow)
        postResponse = await db.post.arrayPostDetail(allPosts.slice(0, postIndex))
        shortVideosResponse = await db.post.arrayPostDetail(shortVideos.slice(0, 10))
